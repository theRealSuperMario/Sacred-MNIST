...
def confnet_config():
    ...
    ex.add_config({"config" : yaml.load(open(config_file))})
...
def define_and_train(batch_size, epochs,
    ...
    def now_string():
        ...
        return '{:%Y%m%d_%H%M%S}'.format(d)
    writer = tf.summary.FileWriter(os.path.join("outputs", now_string()))
    #
    ...
    for i in range(10):
        writer.add_summary(
            tf.Summary(
            value=[tf.Summary.Value(simple_value=0.5, tag='mi_estimate')]
            ), i)
    ...
    # writer.add_summary(
    #     tf.Summary(
    #     value=[tf.Summary.Value(simple_value=score[0], tag='mi_estimate')]
    #     ), epoch)